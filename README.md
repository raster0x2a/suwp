...
- 解除コードは自動生成。リロードしても明示的に再生成しない限り変わりません
...
3. 「共有URLを生成」を押します。ページをリロードしても、解除コードを再生成しない限り同じコードで暗号化されます。
...
- 作成中の解除コードは、リロードしても変わらないように、このタブの `sessionStorage` に保存します。長期保存はせず、通常はタブを閉じると消えます。
